...
// Standalone demo (separate binary from NeuroGame): thin wrapper around IzhikevichModule
// (src/izhikevich_module.cpp), which holds the actual logic and rendering so it can also run
// inside MultiDemo's combined view. See izhikevich_module.hpp for what this demo shows.
...
int main() {
    ...
    IzhikevichModule demo;
    // Reserve room at the bottom for DrawSponsorLogos/DrawFondecytCredit (so the module's own
    // bottom-anchored content -- controls row, "Volver" -- doesn't render underneath them)
    // and a little padding at the top so nothing touches that edge either.
    demo.setBounds({
