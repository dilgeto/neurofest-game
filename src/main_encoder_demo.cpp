...
// Standalone demo (separate binary from NeuroGame): thin wrapper around EncoderModule
// (src/encoder_module.cpp), which holds the actual logic and rendering so it can also run
// inside MultiDemo's combined view. See encoder_module.hpp for what this demo shows.
...
int main() {
    ...
    InitWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Codificadores: de número a espigas");
    int targetMonitor = 0;
